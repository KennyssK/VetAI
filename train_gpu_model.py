...
# --- НАСТРОЙКА GPU И СМЕШАННОЙ ТОЧНОСТИ ---
...
if gpus:
    try:
        ...
        # Включаем Mixed Precision для использования Tensor Cores (RTX 3060/4060)
        policy = mixed_precision.Policy('mixed_float16')
        mixed_precision.set_global_policy(policy)
        print(f"--- Mixed Precision ACTIVE: {policy.compute_dtype} ---")
    except RuntimeError as e:
        print(e)

# --- КОНФИГУРАЦИЯ v15.1 (Optimized) ---
...
BATCH_SIZE = 1024 # На RTX 3060 с Mixed Precision можно пробовать 2048
EPOCHS = 100
...
print(f"Сохранение имен признаков ({len(feature_names)} шт.)...")
joblib.dump(feature_names, FEATURES_PATH)

# 2. Подготовка признаков
print("2. Масштабирование...")
...
# 5. Архитектура (Optimized for Tensor Cores)
def build_robust_model(input_dim, num_classes):
    ...
    model = keras.Sequential([
        ...
        # ВАЖНО: Финальный слой выводим в float32 для точности Softmax
        keras.layers.Dense(num_classes),
        keras.layers.Activation('softmax', dtype='float32', name='predictions')
    ])
...
print("\n--- Запуск Robust обучения v15.1 (Mixed Precision) ---")
